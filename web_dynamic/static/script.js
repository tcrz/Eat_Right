$(document).ready(function () {
  ...
  $(".recipes_list, .recipes_search").on("click", ".recipe", function () {
    ...
    $.get("http://0.0.0.0:5001/api/v1/recipe/" + id, function (data) {
      ...
      const popupTemplate =
        '<div class="recipe-view"><div class="head">' +
  ...
  ***************************/
    ...
    $.ajax({
      ...
      success: function (result) {
        if (result.items.length === 0) {
          ...
          const row =
            '<tr class="food-query"><td class="food">' +
            name.toUpperCase() +
            ' <br> <span class="serving"><p>' +
            quantity +
            " " +
            serving +
            "</p></span></td>" +
            ...
            calories +
            "</p>" +
            '<ion-icon name="remove-outline" style="color:red;cursor:pointer;"></ion-icon></div></td></tr>';
  ...
  $(".submittion").click(function () {
    ...
    let url =
      "http://0.0.0.0:5001/api/v1/age/" + gender + "/" + act + "/" + age;
    if (age < 4 || age > 100) {
